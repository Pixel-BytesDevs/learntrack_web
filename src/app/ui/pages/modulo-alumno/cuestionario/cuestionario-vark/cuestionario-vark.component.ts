...
export class CuestionarioVarkComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (cache) {
      try { this.selectedAnswers = JSON.parse(cache); } catch { /* noop */ }
    }
  }

  /** Carga las 16 preguntas con sus 4 alternativas (servicio hace fallback a mocks si falla) */
  loadQuestions(): void {
    this.cuestionarioService.getQuestions().subscribe((data: PreguntaResponse[]) => {
  ...
  get answeredCount(): number {
    return this.cuestionario.filter(q => (this.selectedAnswers[q.preguntaId]?.length ?? 0) > 0).length;
  }
  ...
  private persistSelection(): void {
    this.storageSession.save(KEYS_STORAGE.cuestionario,JSON.stringify(this.selectedAnswers));
  }
